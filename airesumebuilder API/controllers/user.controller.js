...
export const googleSignInAction = async (req, res) => {
  ...
  { userId: user._id, email: user.email }, // ✅ match with resume controller
  ...
  token,
  user: {
    id: user._id,  // 👈 ye important hai for fetching user's data
...
export const verifyAccount = async (request,response,next)=>{
  try{
     ...
     .then(result=>{
        return response.redirect("http://localhost:3001/login");  // your frontend login page
...
export const verifyEmail = async (req, res) => {
    try {
        ...
        // Verification successful - frontend ke sign-in page pe redirect karo
        return res.redirect("http://localhost:3000/sign-in");  // yahan apna frontend url set karo
    } catch (err) {
        console.error(err);
